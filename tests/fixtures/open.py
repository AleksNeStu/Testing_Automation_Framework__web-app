...
class OpenHelper:
    ...
    def open_url(self, full_url):
        ...
        wd = self.app.wd
        if self.is_obj_url_opened(full_url) is True: return
        else: wd.get(full_url)

    def open_link(self, link_text):
        """Open link used click method on web element."""
        wd = self.app.wd
        if self.is_obj_link_opened(link_text) is True: return
        else: wd.find_element_by_link_text(link_text).click()
